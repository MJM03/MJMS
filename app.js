...
function update(){
 ...
 localStorage.setItem('qf_draft_v61',JSON.stringify({state,form:c.f}))
}
...
function save(){const c=calc(),a=JSON.parse(localStorage.getItem('qf_quotes_v61')||'[]'),q={id:state.id,date:new Date().toISOString(),state:JSON.parse(JSON.stringify(state)),form:c.f,total:c.total,h:c.h};localStorage.setItem('qf_quotes_v61',JSON.stringify([q,...a.filter(x=>x.id!==q.id)].slice(0,50)));history();alert('Cotización guardada.')}
