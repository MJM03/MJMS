...
const V='5.4.0',QKEY='mjm_quoteos_quotes_v5',SKEY='mjm_quoteos_settings_v5';
...
let settings={cost:18,sale:32,margin:22,cont:6,business:'MJM Solutions',tagline:'Software · Automatización · IA',email:'',phone:'',infraAdminMargin:25,maintenanceBasic:120,maintenanceBusiness:250,maintenanceManaged:480,weeklyCapacity:24,...JSON.parse(localStorage.getItem(SKEY)||'{}')};
...
const products=[
['landing','Landing page',5,390,300,1500,''],['corporate','Web corporativa',9,890,800,5000,''],['catalog','Catálogo digital',12,1290,1200,5500,''],['ecommerce','Tienda online',18,2490,2000,15000,''],['pwa','PWA / app web',16,2490,2500,12000,''],['dashboard','Dashboard',14,1990,2500,12000,''],['inventory','Sistema de inventario',18,3490,5000,25000,''],['crm','CRM',20,3990,7000,30000,''],['erp','ERP modular',32,7990,15000,80000,''],['mobile','App Android + iOS',24,5990,6000,30000,''],['desktop','Sistema de escritorio',16,3290,3500,18000,''],['desktop_local','Escritorio local / monousuario',10,1590,1500,7000,''],['desktop_multi','Escritorio multiusuario en red',24,4490,5000,22000,''],['pos_desktop','POS de escritorio',14,2990,3000,16000,''],['automation','Automatización',8,990,1000,10000,''],['ai','Solución con IA',18,3490,5000,40000,'']
...
function blank(){return{id:uid(),name:'Nueva solución digital',client:'',company:'',country:'PE',profile:'portfolio',productivity:'mjm',product:'landing',complexity:'basic',reuse:'high',urgency:'normal',screens:1,modules:['responsive','contact','whatsapp','seo'],objective:'Presentar servicios y generar contactos.',status:'Borrador',infraMode:'direct',maintenancePlan:'none',infraCustom:[],created:new Date().toISOString()}}
function durationFromHours(hours){
 const capacity=Math.max(8,Number(settings.weeklyCapacity||24));
 const weeks=hours/capacity;
 if(hours<=8)return '1–2 días hábiles';
 if(hours<=16)return '2–4 días hábiles';
 if(hours<=28)return '4–7 días hábiles';
 if(weeks<=1.5)return '1–2 semanas';
 if(weeks<=2.6)return '2–3 semanas';
 if(weeks<=4.2)return '3–5 semanas';
 if(weeks<=6.5)return '1–2 meses';
 if(weeks<=10)return '2–3 meses';
 return `${Math.max(3,Math.round(weeks/4))}–${Math.max(4,Math.ceil(weeks/4)+1)} meses`;
}
function calc(q){
 const p=product(q.product), selected=modules.filter(m=>(q.modules||[]).includes(m.id));
 const baseHours=Number(p.base||0);
 const moduleHours=selected.reduce((a,b)=>a+b.hours,0);
 const screensHours=Math.max(0,(Number(q.screens||1)-1)*1.2);
 const raw=baseHours+moduleHours+screensHours;
 const complexityFactor=({basic:1,medium:1.18,high:1.42,enterprise:1.72}[q.complexity]||1);
 const reuseFactor=({high:.75,medium:.88,low:.96,none:1}[q.reuse]||1);
 const productivityFactor=({student:1.28,junior:1,semi:.88,studio:.82,mjm:.78}[q.productivity||'mjm']||1);
 const urgencyFactor=({normal:1,fast:1.08,urgent:1.18}[q.urgency]||1);
 let h=Math.max(5,Math.round(raw*complexityFactor*reuseFactor*productivityFactor*urgencyFactor));
 const qa=Math.max(2,Math.round(h*.10)), coordination=Math.max(1,Math.round(h*.05));
 h+=qa+coordination;
 const mk=markets[q.country]||markets.PE, mult=mk.m[q.profile]||1;
 const internal=h*settings.cost, commercial=h*settings.sale*mult;
 const min=internal*(1+settings.margin/100)+commercial*settings.cont/100;
 const rec=Math.max(min,commercial,p.floor*mult), fx=mk.fx;
 return{
  p,selected,h,duration:durationFromHours(h),currency:mk.currency,
  internal:internal*fx,min:min*fx,
  express:Math.round(Math.max(min,rec*.82)*fx/10)*10,
  rec:Math.round(rec*fx/10)*10,premium:Math.round(rec*1.35*fx/10)*10,
  low:p.low*fx,high:p.high*fx,
  breakdown:{base:Math.round(baseHours*complexityFactor*reuseFactor*productivityFactor),modules:Math.round(moduleHours*complexityFactor*reuseFactor*productivityFactor),screens:Math.round(screensHours*complexityFactor*reuseFactor*productivityFactor),qa,coordination,raw:Math.round(raw)}
 };
}
...
function diagnose(){
 ...
 current.modules=[...new Set(mods)];if(pid==='pos_desktop'){
   current.modules=[...new Set(['auth','roles','db','crud','scanner','printing','localdb','installer','backup',...(offline.value==='Sí'?['offline']:[]),...(u>=5?['network']:[])])];
   current.screens=7;
 }if(pid!=='pos_desktop')current.screens=Math.max(1,Math.min(20,Math.ceil((text.split(/[,; ]+/).filter(Boolean).length)/8)+2));current.complexity=u>=100||+integrations.value>=4?'high':u>=20||+integrations.value>=2?'medium':'basic';current.reuse='high';current.productivity='mjm';
...
function route(){
  ...
  <div class="route-banner card">
    <div><span class="eyebrow">Cotización activa</span><h2>${esc(q.name)}</h2><p>${a.p.name} · Complejidad tecnológica ${a.level} · ${c.duration}</p></div>
...
function architect(){
  ...
  <div class="grid g3">
    ...
    <div class="card kpi"><span class="muted">Inversión recomendada</span><b>${money(c.rec,c.currency)}</b><small>${c.duration}</small></div>
  </div>
  <div class="grid g2" style="margin-top:18px">
...
function proposalHTML(q){
  ...
  <div class="meta"><div class="box"><small>Solución</small><br><b>${c.p.name}</b></div><div class="box"><small>Plazo</small><br><b>${c.duration}</b></div><div class="box"><small>Modalidad</small><br><b>Precio cerrado</b></div></div>
  ...
  <h2>Inversión de desarrollo</h2><div class="summary"><div class="box"><small>Honorarios MJM Solutions</small><div class="price">${money(c.rec,c.currency)}</div></div><div class="box"><small>Plazo estimado</small><b>${c.duration}</b></div><div class="box"><small>Garantía</small><b>15 días</b></div></div>
