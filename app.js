...
function calc(){
 ...
 const margin=basePrice?((basePrice-internal)/basePrice)*100:0,weeks=3;
...
function phases(c){return [
 {name:'Análisis y definición',desc:'Validación del alcance, estructura de datos y flujos principales.',h:Math.round(c.h*.15),label:'Semana 1'},
 {name:'Diseño y base técnica',desc:'Interfaz, navegación, autenticación y estructura del sistema.',h:Math.round(c.h*.20),label:'Semana 1'},
 {name:'Desarrollo funcional',desc:'Productos, categorías, banners, blog, usuarios, analítica e importación.',h:Math.round(c.h*.45),label:'Semanas 2–3'},
 {name:'Pruebas y ajustes',desc:'Validación funcional, correcciones y revisión con el cliente.',h:Math.round(c.h*.12),label:'Semana 4'},
 {name:'Capacitación y producción',desc:'Publicación, guía de uso, capacitación y entrega final.',h:Math.round(c.h*.08),label:'Semana 4'}
]}
...
function update(){
 ...
 localStorage.setItem('qf_draft_v613',JSON.stringify({state,form:c.f}))
}
...
function save(){const c=calc(),a=JSON.parse(localStorage.getItem('qf_quotes_v613')||'[]'),q={id:state.id,date:new Date().toISOString(),state:JSON.parse(JSON.stringify(state)),form:c.f,total:c.total,h:c.h};localStorage.setItem('qf_quotes_v613',JSON.stringify([q,...a.filter(x=>x.id!==q.id)].slice(0,50)));history();alert('Cotización guardada.')}
function history(){const a=JSON.parse(localStorage.getItem('qf_quotes_v613')||'[]');$('#historyList').innerHTML=a.length?a.map(q=>`<div class="historyItem" data-id="${q.id}"><div><b>${q.form.project||'Proyecto sin nombre'}</b><small>${q.form.client||'Cliente por confirmar'} · ${new Date(q.date).toLocaleString('es-PE')} · ${money(q.total)}</small></div><div><button data-load>Cargar</button><button data-del>Eliminar</button></div></div>`).join(''):'<p>No hay cotizaciones guardadas.</p>';$$('.historyItem').forEach(r=>{const q=a.find(x=>x.id===r.dataset.id);r.querySelector('[data-load]').onclick=()=>load(q);r.querySelector('[data-del]').onclick=()=>{localStorage.setItem('qf_quotes_v613',JSON.stringify(a.filter(x=>x.id!==q.id)));history()}})}
